...
import { visibleOperatorLinks } from "./NavConfig";
...
export function UserMenu(): JSX.Element | null {
  ...
  const renderedOperatorLinks = visibleOperatorLinks(session)
    .map((link) => {
      if (link.id === "open-admin") {
        ...
        return (
          <a
            ...
            role="menuitem"
            href={adminUrl}
            className="hc-usermenu__item"
            data-testid="usermenu-admin-link"
            target="_blank"
            rel="noreferrer noopener"
          >
            {link.label}
            <span className="hc-usermenu__item-hint">{link.description}</span>
          </a>
        );
      }
      return null;
    ...
    <div className="hc-usermenu" ref={containerRef}>
      ...
      {open && (
        <div role="menu" id={menuId} aria-labelledby={triggerId} className="hc-usermenu__panel">
          ...
          {renderedOperatorLinks.length > 0 && (
            <div className="hc-usermenu__group" role="presentation">
              <div className="hc-usermenu__group-label">Operator</div>
              {renderedOperatorLinks}
            </div>
          )}
          <div className="hc-usermenu__group" role="presentation">
            <button
