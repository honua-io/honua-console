...
/**
 * Single application chrome: top bar, four-area sidebar driven by `NavConfig`,
 * and a content slot. Route components never re-render the chrome — they
 * render into the `<main>` slot.
 */
    ...
    <div className="hc-shell" data-nav-open={navOpen ? "true" : "false"}>
      ...
      <header className="hc-topbar">
        ...
        <div className="hc-topbar__search-slot" aria-hidden="true">
          {/* Reserved for global search (later ticket). */}
        </div>
        <UserMenu />
      </header>
      ...
      <div className="hc-shell__body">
        <nav id="hc-sidenav" className="hc-sidenav" aria-label="Primary" data-state={navOpen ? "open" : "closed"}>
          {navGroups.map((group) => (
            <div key={group.section} className="hc-sidenav__group" data-section={group.section}>
              <div className="hc-sidenav__group-label">{group.label}</div>
