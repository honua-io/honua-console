...
/operate/copilot               Copilot Findings · the ops-copilot information + approval seat (#193): lists honua-server's DETERMINISTIC ops findings (GET /api/v1/admin/observability/findings) with severity chip, plain-language explanation, rule id, subject, and evidence refs; a finding's recommended action is proposed by id (POST .../findings/{id}/propose) through the SAME governed approval gateway the inbox witnesses — ProposalCreated surfaces the proposalId with a link to /inbox; a cleared condition (404) refreshes the list with a notice. NO model/LLM anything (ADR-0028). Empty state = all conditions healthy; else missing-binding/forbidden/unsupported/unavailable
