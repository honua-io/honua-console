...
| `/studio/map` | `auth` | missing-binding when no server base address; server-bound map builder on the Studio package lifecycle (honua-server#1180) + content publication registry (honua-server#1183) — author layers/filters/style/popup/legend/basemap/extent/interactions, save a `map.package` draft, review then publish (freezes an immutable content version and routes it to the publication contract), and reopen a published version as a new draft; bound-but-no-list capability state (no Studio package list route — open by id or create new); publish blocked until title, a sourced layer, basemap, and initial extent are present | unauth-redirect / missing-permission (server RBAC) | studio |
| `/studio/dashboard` | `auth` | missing-binding when no server base address; empty dashboard-package list when bound; server-bound builder (honua-console#55) with data bindings, layout panels, Vega-Lite charts, filters, narrative, version pinning, and responsive preview; publish blocked until every chart declares a Vega-Lite spec and every panel binds to a declared data binding | unauth-redirect / missing-permission (server RBAC) | studio |
| `/studio/report` | `auth` | missing-binding when no server base address; server-bound report builder (honua-console#56) read path on the content publication registry (honua-server#1183) — open a published report to inspect its server-owned route, visibility/embed policy, and immutable version history; authoring (narrative, data bindings, Vega-Lite charts, map panels, tables, filters, responsive preview) and publish/share/embed land in follow-up slices | unauth-redirect / missing-permission (server RBAC) | studio |
...
| Unsupported package binding | `ConsoleStateView Kind="unsupported-package"` | generated-app, generated Studio package, or saved-map package newer than Console runtime understands | used on `/studio/apps/:itemId/preview`, `/studio/map`, `/studio/app`, `/maps/:mapId`, and `/embed/maps/:mapId` (`/studio/form`, `/studio/query`, `/studio/analysis`, `/studio/dashboard`, and `/studio/report` are server-bound and render their own missing-binding / capability-state surface instead) |
