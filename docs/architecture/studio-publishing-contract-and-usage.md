...
| Dashboard | `/studio/previews/:draftId` or `/studio/drafts/:draftId` | `/dashboards/:itemId` | Fixture dashboard projection with Vega-Lite specs |
| Report | `/studio/previews/:draftId` or `/studio/drafts/:draftId` | `/reports/:itemId` | Fixture report projection |
...
`title`, `summary`, and `versionNote` are required publish metadata. The UI marks them required, and the fixture client rejects blank values for non-UI callers before creating or updating a published item.
...
Each submit attempt clears any previous successful publish result before client-side validation or fixture submission. A failed retry leaves only the current inline problem state visible, so stale Catalog, preview, Share, Embed, or Edit in Studio links from an earlier success are not presented as the outcome of the latest attempt.
...
Console uses the shared publish problem taxonomy for Studio draft list, draft, preview, review, publish, published route, and reopen failures:
