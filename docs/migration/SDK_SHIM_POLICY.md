...
| Analysis Content HTTP shim (`IStudioAnalysisContentClient`/`HttpStudioAnalysisContentClient`, the `AnalysisContent*`, `AnalysisPlan*`, `AnalysisJob*`, `SavedQuery*`, and artifact wire records) in `src/Honua.Console.Contracts/AnalysisContentShims.cs` | .NET | honua-console#53 | honua-sdk-dotnet#166 projecting the Analysis Content and analysis package DTOs (honua-server#1182 merged on server trunk; no consumable SDK projection yet) | Studio | honua-console#7 swaps to the SDK Analysis Content client when projected |
