...
| Analysis Content saved-query HTTP shim (`IAnalysisContentClient`/`AnalysisContentHttpClient`, the `AnalysisContent*`, `SavedQuery*`, `FilterPlan*`, and `AnalysisArtifact*` wire records) in `src/Honua.Console.Contracts/AnalysisContentShims.cs` | .NET | honua-console#52 | honua-sdk-dotnet#170 projecting the Analysis Content and saved-query DTOs (honua-server#1182 merged on server trunk; no consumable SDK projection yet) | Studio | honua-console#7 swaps to the SDK Analysis Content client when projected |
