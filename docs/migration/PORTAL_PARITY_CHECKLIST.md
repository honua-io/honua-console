...
Status: filed 2026-05-23; reconciled 2026-05-24 for
`honua-console#34` catalog/share route-slice behavior.
...
| Content Catalog | `/catalog` browse/search requires a signed-in workspace session and accepts only `q`, `type`, `tag`, `owner`, `visibility`, `sort`, `cursor`; detail lives at `/catalog/:idOrSlug` with `tab=overview|versions|lineage|bindings|publication|permissions|activity|usage`. Supported type strip values: `dataset`, `service`, `layer`, `document`, `map`, `dashboard`, `report`, `form`, `app`, `workflow`, `analysis`, `gp-service`, `etl-pipeline`, `connector`, `template`. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/catalog.smoke.md`, `honua-portal/smoke/catalog-browser.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/CatalogSearchStateTests.cs` |
| Map Viewer (incl. Maputnik saved-map style editor) | `/maps/:id` viewer and authenticated `/maps/new?from=:itemId` draft hydration; draft hydration is limited to supported service/layer source items and continues to Studio at `/studio?source=catalog&itemId=:itemId`. Anonymous public-link reads use `?token=<value>` on `/maps/:id`, while signed-in sessions use the authenticated read path even if a stale token is present. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/docs/features/MAP_VIEWER_MVP.md`, `honua-portal/smoke/maputnik-style-editor.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
| Saved Web Maps | `/maps/:id` saved-map package read and viewer route are in the #34 slice; viewer edit actions reopen Studio at `/studio?source=map&itemId=:mapId`. Full save/load/duplicate/rename/delete and portal-override style separation remain in the broader saved-map port. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/saved-maps.smoke.md`; `honua-portal/docs/contracts/webmap-doc-v1.md`; Console #9 |
| Sharing And Embeds | Catalog publication tab builds canonical share links (`/maps/:id?token=` for maps, `/catalog/:id?token=` for non-maps); `/embed/maps/:id` accepts `chrome`, `legend`, `zoom`, `extent`, allows tokenless public embeddable maps, requires bearer placement in `#embedToken=` for token-authorized embeds, and rejects query-string `token` / `embedToken`. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/sharing-and-embed.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
...
| Open Data Pages | `/share` and `/share/public` collection routes plus `/public` compatibility alias; `/share/public/items/:idOrSlug` and `/public/items/:idOrSlug` detail for public open-data service/layer/document items only. DCAT-US 3.0, JSON-LD, related items, and revision history remain in the full open-data parity work. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/open-data.smoke.md`; `honua-portal/docs/features/open-data-publishing-quality.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
| Collaboration (annotations and comments) | Future viewer annotation workspace on `/maps/:id`; embed read-only with public-moderation seam | [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/annotations-comments.smoke.md`; `honua-portal/docs/features/ANNOTATIONS_COMMENTS.md`; Console #9 |
| Collaborative Map Editing (MVP contract) | Future viewer collaboration panel + presence/cursor/lock seam on `/maps/:id` | [honua-console#4](https://github.com/honua-io/honua-console/issues/4) (MVP contract only; WebSocket/CRDT GA tracked separately) | in-progress | `honua-portal/docs/features/COLLABORATIVE_MAP_EDITING.md`; Console #9 |
