...
| Sharing And Embeds | Catalog publication tab builds canonical share links (`/maps/:id?token=` for maps, `/catalog/:id?token=` for non-maps); `/embed/maps/:id` accepts `chrome`, `legend`, `zoom`, `extent`, and requires bearer placement in `#embedToken=`. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/sharing-and-embed.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
