...
| Map Viewer (incl. Maputnik saved-map style editor) | `/maps/:id` viewer and authenticated `/maps/new?from=:itemId` draft hydration; anonymous public-link reads use `?token=<value>` on `/maps/:id`, while signed-in sessions use the authenticated read path even if a stale token is present. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/docs/features/MAP_VIEWER_MVP.md`, `honua-portal/smoke/maputnik-style-editor.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
| Saved Web Maps | `/maps/:id` saved-map package read and viewer route are in the #34 slice; full save/load/duplicate/rename/delete and portal-override style separation remain in the broader saved-map port. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/saved-maps.smoke.md`; `honua-portal/docs/contracts/webmap-doc-v1.md`; Console #9 |
| Sharing And Embeds | Catalog publication tab builds canonical share links (`/maps/:id?token=` for maps, `/catalog/:id?token=` for non-maps); `/embed/maps/:id` accepts `chrome`, `legend`, `zoom`, `extent`, allows tokenless public embeddable maps, and requires bearer placement in `#embedToken=` for token-authorized embeds. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/sharing-and-embed.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
