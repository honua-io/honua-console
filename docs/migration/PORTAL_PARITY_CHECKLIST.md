...
| Content Catalog | `/catalog` browse/search requires a signed-in workspace session and accepts only `q`, `type`, `tag`, `owner`, `visibility`, `sort`, `cursor`; detail lives at `/catalog/:idOrSlug` with overview, versions, lineage, bindings, publication, permissions, activity, and usage tabs. Supported type strip values: `dataset`, `service`, `layer`, `document`, `map`, `dashboard`, `report`, `form`, `app`, `workflow`, `analysis`, `gp-service`, `etl-pipeline`, `connector`, `template`. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/smoke/catalog.smoke.md`, `honua-portal/smoke/catalog-browser.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/CatalogSearchStateTests.cs` |
| Map Viewer (incl. Maputnik saved-map style editor) | `/maps/:id` viewer and authenticated `/maps/new?from=:itemId` draft hydration; anonymous public-link reads use `?token=<value>` on `/maps/:id`. | [honua-console#34](https://github.com/honua-io/honua-console/issues/34) route slice under [honua-console#4](https://github.com/honua-io/honua-console/issues/4) | in-progress | `honua-portal/docs/features/MAP_VIEWER_MVP.md`, `honua-portal/smoke/maputnik-style-editor.smoke.md`; Console #9; `tests/Honua.Console.Native.Core.Tests/ConsoleCatalogClientTests.cs` |
