...
| ArcGIS Pro-style capability breadth through natural language | GIS capability registry, tool/workflow aliases, parameter schema, environment settings, license/extension status, execution target, support status, input/output types, validation, cost/runtime estimate, and provenance mapping | Studio package backlog covers editors; GP/ETL backlog covers workflow execution substrate | Add backend/SDK work for a discoverable capability registry so NL can plan across Honua-native tools, ArcGIS/Esri-compatible operations, remote GP/process services, and supported custom script/model tools without cloning the ArcGIS Pro UI. |
...
| Survey-style form builder | Form package, field/domain rules, conditional visibility, attachments, offline policy, submission target, sync policy | `honua-server#1184` covers the form package lifecycle/offline policy bound by `/studio/form`; `honua-server#1158` covers back-office parity; temporal/sync tickets cover later conflict review | Keep submission ingestion, attachment storage, audit evidence, and SDK projections server-owned; Console must continue binding the builder through the temporary contract shim until the SDK projection lands. |
...
| Operate event viewer, logs, telemetry, alerts, jobs | Normalized operational event query, server telemetry status, log/evidence links, alert/rule APIs, job run API | `honua-server#1168`, `#1169`, `#1170`; `honua-sdk-js#229`; `honua-sdk-dotnet#231`; `honua-console#24` | Console now binds the read-only Operate page to live admin APIs through a thin .NET shim; SDK projection replacement remains active. AI summaries remain evidence-linked and advisory. |
...
| Native Console multi-environment/mTLS | Environment profile, advertised transport capabilities, native gRPC streaming, client certificate trust profile | `honua-server#1171`; `honua-console#26`; `honua-console#44`; `honua-sdk-dotnet#166` | `#44` binds trust validation to honua-server#1171 through `Honua.Console.Contracts`; web renders native-only capability states without depending on MAUI until the SDK projection replaces the shim. |
...
| P1 | [honua-sdk-dotnet#231](https://github.com/honua-io/honua-sdk-dotnet/issues/231) | `sdk-dotnet-operate-observability-admin-client` | Replace Console's temporary `OperateObservabilityContracts.cs` shim with typed .NET SDK clients for server overview/status, events, logs, alerts, rules, zones, jobs, and investigations. |
