...
| Generic approve/reject parity | Existing generic proposal and deploy clients use the same operator-only transport as reads | Server #3304 publication-intent bridge and #3365 scoped approval grant are still open; run separate Console/CLI human decisions and compare canonical identifiers after their implementation |
