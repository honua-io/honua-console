...
The shared Razor shell currently exposes the first Console-native package editor set at `/studio/query`, `/studio/analysis`, `/studio/map`, `/studio/dashboard`, `/studio/report`, `/studio/form`, and `/studio/app`. `/studio/analysis` is **server-bound** (`honua-console#53`): it renders the `StudioAnalysisBuilder` against the honua-server Analysis Content API (`honua-server#1182`) through the `IStudioAnalysisContentClient` shim — author/preview a saved query, submit an analysis package as a job, observe the result artifact, and hand the binding downstream; unbacked slivers (compute estimate, package dry-run, NL→plan) render explicit "unavailable" states, never a mock. The other six per-editor families (`honua-console#52`, `#54`–`#58`) still use the local `studio-package-mock/v1` lifecycle projection documented in [Studio Package Editor Routes](docs/studio/package-editor-routes.md) and surface a missing-binding state for validate/preview rather than mock success; they bind to the honua-server package lifecycle on their own tickets. The shared `/studio` shell already binds that lifecycle (`honua-server#1180`/`#1181`).
