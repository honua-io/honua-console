...
The shared Razor shell currently exposes the first Console-native package editor set at `/studio/query`, `/studio/analysis`, `/studio/map`, `/studio/dashboard`, `/studio/report`, `/studio/form`, and `/studio/app`. `/studio/query` is **server-bound** (`honua-console#52`): it renders `SavedQueryEditorPage` against the honua-server Analysis Content API (`honua-server#1182`) through the Analysis Content shim, and renders a missing-binding state instead of mock query data when no server base address is configured. The other six per-editor families (`honua-console#53`–`#58`) still use the local `studio-package-mock/v1` lifecycle projection documented in [Studio Package Editor Routes](docs/studio/package-editor-routes.md) and surface a missing-binding state for validate/preview rather than mock success; they bind to the honua-server package lifecycle on their own tickets. The shared `/studio` shell already binds that lifecycle (`honua-server#1180`/`#1181`).
