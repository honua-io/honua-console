...
The current `/studio` route includes the first shared Razor package shell slice for `honua-console#38`; the same shell is also mounted for `/studio/proof`, source-scoped `/studio/drafts?source=<kind>&id=<itemId>` starts, and `/studio/apps/:itemId/preview` reopen paths. It lets a builder choose a workflow, submit a prompt, answer structured clarification questions, inspect the active package, and move that package through Draft, Preview, Saved version, and Published UI states. The shell records the temporary Console-owned `studio-authoring-shell/v1` projection with `package-shell/v1` package snapshots until the server package lifecycle API and SDK helpers are wired in.
